...
import mathematics from '../data/maths';
...
function Maths() {
  ...
  const filteredMaths = currentElemets.filter((maths) => {
    return maths.description.toLowerCase().includes(search.toLowerCase());
  });

  function load() {
    setCurrentElemets(mathematics.slice(0, count + 10));
  ...
  return (
    <div className="general-container">
      ...
      {filteredMaths.length === 0 && <EmptyIndicator text="No Data" />}

      {filteredMaths.map((maths) => (
        <SpecificGeneral
          key={maths.title}
          title={maths.title}
      ...
      {filteredMaths.length > 0 && count < mathematics.length && (

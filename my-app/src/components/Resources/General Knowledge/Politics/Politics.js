...
import politicsTopics from '../data/politics';
...
function Politics() {
  ...
  const filteredPolitics = currentElemets.filter((politics) => {
    return politics.description.toLowerCase().includes(search.toLowerCase());
  });

  function load() {
    setCurrentElemets(politicsTopics.slice(0, count + 10));
  ...
  return (
    <div className="general-container">
      ...
      {filteredPolitics.length === 0 && <EmptyIndicator text="No Data" />}

      {filteredPolitics.map((maths) => (
        <SpecificGeneral
          key={maths.title}
          title={maths.title}
      ...
      {filteredPolitics.length > 0 && count < politicsTopics.length && (

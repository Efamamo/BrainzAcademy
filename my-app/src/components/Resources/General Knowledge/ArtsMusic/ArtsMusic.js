...
function ArtAndMusic() {
  ...
  const filteredArts = currentElemets.filter((art) => {
    return art.description.toLowerCase().includes(search.toLowerCase());
  });

  function load() {
    setCurrentElemets(artAndMusic.slice(0, count + 10));
  ...
  return (
    <div className="general-container">
      ...
      {filteredArts.length === 0 && <EmptyIndicator text="No Data" />}
      ...
      {filteredArts.length > 0 && count < artAndMusic.length && (
